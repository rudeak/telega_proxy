...
def change_header (page, id):
    ...
    header_soup.find('h1')[0] = '<h1><a href ={}>{}</a></h1>'.format('/proxy'+str(id), game.game_name)
    print (header_soup)
    return header_soup
"""
<ul>
...
def get_sectors_count(pageSoup):
    ...
    try:
        ...
        sectors_all = [
            str(s) for s in sectors_count.get_text().split() if s.isdigit()][0]
...
def get_sectors_info(pageSoup):
    ...
    try:  # заглушка якщо один код на рівні TODO переробити !!!!
        ...
        sectors_list.append(
            {'name': 'Sector 1', 'entered': False, 'answer': '', 'gamer': ''})
    ...
    for sector in sectors:
        ...
        sectors_list.append(
            {'name': name, 'entered': entered, 'answer': answer, 'gamer': gamer})
    ...
    for prompt in prompts:
        ...
        if len(prompt.findAll('span', class_=code_not_entered_class)) != 0:
            if str(prompt).find('Penalty') < 0:
                jprompt.append({'number': counter, 'text': '',
                                'timer': get_timer(prompt.prettify())})
        else:
            jprompt.append(
                {'number': counter, 'text': str(prompt), 'timer': ''})
...
def get_bonuses(pageSoup):
    ...
    for bonus in bonuses:
        ...
        if len(bonus.findAll('span', class_=code_not_entered_class)) != 0:
            jbonus.append({'number': counter, 'text': '',
                           'bonus_text': '', 'completed': False, 'passed': True})
        else:
            if len(bonus.findAll('h3', class_=code_entered_class)) != 0:
                if len(bonus.findAll('p')) != 0:
                    jbonus.append({'number': counter, 'text': str(bonus.h3), 'bonus_text': str(
                        bonus.p), 'completed': True, 'passed': False})
                else:
                    jbonus.append({'number': counter, 'text': str(
                        bonus.h3), 'bonus_text': '', 'completed': True, 'passed': False})
            else:
                jbonus.append({'number': counter, 'text': str(
                    bonus), 'bonus_text': '', 'completed': False, 'passed': False})
...
def set_block(html):
    ...
    html = rename_block(''.join(html_dic))
    #html = ''.join(html_dic)
...
def rename_block(html_dic):
    ...
    for z in range(0, len(html_dic)-1):
        ...
        if html_dic[z] == '<div class="block">' and counter == 1:
            ...
            counter = 2
        if html_dic[z] == '<div class="block">' and counter == 2 and (html_dic[z+1].strip() != '<h3 class="'+correct_bonus_class+'">' or html_dic[z+1].strip() != '<h3 class="'+code_entered_class+'">'):
...
def get_code_date(inStr):
    ...
    if inStr.find('/') > 0:
        tmp_date_str = str(datetime.now().year)+'/' + \
        ...
        tmp_date_str = str(datetime.now().year)+' ' + str(datetime.now().day)+' '+str(
            datetime.now().month)+' ' + inStr.split()[0]+' ' + inStr.split()[1]
        try:
